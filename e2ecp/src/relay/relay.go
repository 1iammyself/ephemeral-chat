...
type IncomingMessage struct {
	...
	Recipients        []string `json:"recipients,omitempty"`         // Specific recipients
}
	...
	for {
		...
		default:
			...
			out := OutgoingMessage{
				...
				Recipients:        in.Recipients,        // Forward recipients list
			}

			room.Mutex.Lock()
			for _, other := range room.Clients {
				if other.ID != client.ID {
					// Check if specific recipients are targeted
					if len(out.Recipients) > 0 {
						...
						// If user is not in the list, skip sending
						if !isRecipient {
							continue
						}
					}
