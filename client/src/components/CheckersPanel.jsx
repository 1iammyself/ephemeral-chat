...
export default function CheckersPanel({ message, currentUser, roomVibe }) {
  ...
  useEffect(() => {
    ...
    const tid = setTimeout(() => {
      const move = getCpuMove(board, gameData.cpu.difficulty, 2);
      if (move) socketManager.emit('checkers-move', { messageId, move });
  ...
  return (
    <div className="flex flex-col items-center p-3 h-full gap-3 overflow-y-auto">
      ...
      {/* Board — fills available width, square cells via aspect-ratio */}
              ...
              return (
                <div
                  ...
                  className={`aspect-square flex items-center justify-center transition-all relative
                    ${isDark && (isMyTurn && status==='playing') ? 'cursor-pointer' : ''}`}
                  style={{
                    background: isSelected ? accentColor + '66' : isDark ? darkSq : lightSq,
                  }}
                >
                  {isTarget && !cell && (
