...
const JoinRoomModal = ({ roomCode, onJoin, onCancel, error, isProcessingInvite = false, isWaitingForHost = false }) => {
  ...
  if (isWaitingForHost) {
    // isProcessingInvite stays true from knock → knock-approved → join-room callback.
    // Once the host approves and performJoin fires, we show "Connecting…" instead of
    // "Waiting for Host" so the user knows something is happening.
    const approved = isProcessingInvite;
    return (
