...
import { Music, Plus, X, ChevronRight, Play, List } from 'lucide-react';
...
export default function CollabPlaylist({ isOpen, onClose, isHost, currentUser, roomCode }) {
  ...
  // Sync: on playlist-playing, start playback at the right offset
  useEffect(() => {
    const handlePlaying = ({ url, startedAt, title }) => {
      setQueue(q => {
        const idx = q.findIndex(t => t.url === url);
        setCurrentIndex(idx >= 0 ? idx : currentIndex);
        return q;
      });
      // Audio playback sync
      if (audioRef.current) {
  ...
  const addUrl = useCallback(() => {
    ...
    const title = detected ? (detected.type === 'youtube' ? `YouTube: ${detected.id}` : url) : url;
